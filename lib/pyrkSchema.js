							...
							else
							{
								...
								if (failed === false)
								{
									...
									else
									{
										...
										await qdb.createJournalEntry(transactionData.txid, blockData.hash, blockData.height, (new Date(transactionData.time * 1000)).toJSON(), transactionData.time, 'update', 'addresses', {"recordId": srecordId }, {"tokenBalance": totalsenderbalance.toFixed(8), "lastUpdatedBlock": blockData.height });
										...
										else 
										{
											...
											await qdb.createJournalEntry(transactionData.txid, blockData.hash, blockData.height, (new Date(transactionData.time * 1000)).toJSON(), transactionData.time, 'update', 'addresses', {"recordId": rrecordId }, {"tokenBalance": totalsenderbalance.toFixed(8), "lastUpdatedBlock": blockData.height });
							...
							else
							{
								...
								else
								{
									...
									if (failed === false)
									{
										...
										await qdb.createJournalEntry(transactionData.txid, blockData.hash, blockData.height, (new Date(transactionData.time * 1000)).toJSON(), transactionData.time, 'update', 'addresses', {"recordId": recordId }, {"tokenBalance": totalsenderbalance.toFixed(8), "lastUpdatedBlock": blockData.height });
							...
							else
							{
								...
								else
								{
									...
									if (failed === false)
									{
										...
										await qdb.updateDocument('tokens', {"tokenDetails.tokenIdHex": tokenId }, {"tokenDetails.ownerAddress": contractData.newowner, "lastUpdatedBlock": blockData.height, "tokenStats.qty_valid_txns_since_genesis": newValidTxns});

										await qdb.createJournalEntry(transactionData.txid, blockData.hash, blockData.height, (new Date(transactionData.time * 1000)).toJSON(), transactionData.time, 'update', 'tokens', {"tokenDetails.tokenIdHex": tokenId }, {"tokenDetails.ownerAddress": contractData.newowner, "lastUpdatedBlock": blockData.height, "tokenStats.qty_valid_txns_since_genesis": newValidTxns});
						...
						else if (contractData.transactiontype  == 'ADDMETA')
						{
							...
							else if (metaCode == 0)
							{
								...
								var TransactionObject = {
									...
									blockHeight: blockData.height,
									valid: false,
									invalidReason: 'Token AddMeta Failed - Invalid MetaCode',
									transactionDetails: TransactionDetails
								}
							...
							else
							{
								...
								if (failed === false)
								{
									...
									else if ((metaCode == 1 || metaCode == 2) && findSenderAddress.isOwner == false)
									{
										...
										var TransactionObject = {
											...
											blockHeight: blockData.height,
											valid: false,
											invalidReason: 'Token AddMeta Failed - MetaCode 1 and 2 Reserved for Owner Only',
											transactionDetails: TransactionDetails
										}
										...
										console.log('Error: Sender addresses not authorized for meta code 1 or 2');
									...
									else
									{
										...
										if (metaCode == 1)
										{
											...
											var newDocumentURI = metaData;

											await qdb.updateDocument('tokens', {"tokenDetails.tokenIdHex": tokenId }, {"tokenDetails.documentUri": newDocumentURI, "lastUpdatedBlock": blockData.height, "tokenStats.block_last_active_meta": blockData.height, "tokenStats.qty_valid_txns_since_genesis": newValidTxns, "tokenStats.qty_valid_meta_since_genesis": newTokenMetas });

											await qdb.createJournalEntry(transactionData.txid, blockData.hash, blockData.height, (new Date(transactionData.time * 1000)).toJSON(), transactionData.time, 'update', 'tokens', {"tokenDetails.tokenIdHex": tokenId }, {"tokenDetails.documentUri": newDocumentURI, "lastUpdatedBlock": blockData.height, "tokenStats.block_last_active_meta": blockData.height, "tokenStats.qty_valid_txns_since_genesis": newValidTxns, "tokenStats.qty_valid_meta_since_genesis": newTokenMetas });
										...
										else if (metaCode == 2)
										{
											...
											var newLogoURI = metaData;

											await qdb.updateDocument('tokens', {"tokenDetails.tokenIdHex": tokenId }, {"tokenDetails.logoUri": newLogoURI, "lastUpdatedBlock": blockData.height, "tokenStats.block_last_active_meta": blockData.height, "tokenStats.qty_valid_txns_since_genesis": newValidTxns, "tokenStats.qty_valid_meta_since_genesis": newTokenMetas });

											await qdb.createJournalEntry(transactionData.txid, blockData.hash, blockData.height, (new Date(transactionData.time * 1000)).toJSON(), transactionData.time, 'update', 'tokens', {"tokenDetails.tokenIdHex": tokenId }, {"tokenDetails.logoUri": newLogoURI, "lastUpdatedBlock": blockData.height, "tokenStats.block_last_active_meta": blockData.height, "tokenStats.qty_valid_txns_since_genesis": newValidTxns, "tokenStats.qty_valid_meta_since_genesis": newTokenMetas });
